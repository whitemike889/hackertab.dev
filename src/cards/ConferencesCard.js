...
function ConferencesCard({ label, analyticsTag }) {
  ...
  const renderConfs = (confs) => {
    return confs.map((conf, index) => (
      <ConferenceItem conf={conf} key={`cf-${index}`} index={index} analyticsTag={analyticsTag} />
    ))
  }
  return (
    <CardComponent icon={<HiTicket className="blockHeaderIcon" color="#4EC8AF" />} title={label}>
      <ListComponent fetchData={fetchConfs} renderData={renderConfs} refresh={refresh} />
    </CardComponent>
  )
}
